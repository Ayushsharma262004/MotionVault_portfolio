    ...
    <section id="blog" data-testid="blog-section" className="py-24 md:py-32 lg:py-40 px-6 md:px-12 lg:px-24 bg-empire-black relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-12 gsap-heading">
          <div>
            <span className="text-xs font-body font-bold uppercase tracking-[0.25em] text-empire-red">// Insights</span>
            <h2 data-testid="blog-heading" className="font-heading font-black text-4xl md:text-5xl lg:text-6xl text-white leading-tight tracking-tight mt-4">
              Latest from<br /><span className="text-gradient-red">Empire AI.</span>
            </h2>
          </div>
          <button data-testid="blog-view-all" className="mt-4 md:mt-0 flex items-center gap-2 text-sm font-body font-semibold text-empire-red animated-underline">
          ...
          {posts.map((post, i) => (
            <article key={i} data-testid={`blog-card-${i}`} onClick={() => navigate(`/blog/${post.slug}`)} className="group cursor-pointer">
              <div className="parallax-img-wrap relative rounded-2xl overflow-hidden aspect-[16/10] mb-4">
                ...
                <div className="absolute top-4 left-4"><span className="liquid-glass rounded-full px-3 py-1 text-xs font-body font-semibold text-white">{post.category}</span></div>
              </div>
              <div className="flex items-center gap-3 mb-2">
                <span className="text-xs font-body text-white/40">{post.date}</span>
                <span className="text-white/10">|</span>
                <span className="flex items-center gap-1 text-xs font-body text-white/40"><Clock className="w-3 h-3" />{post.readTime}</span>
              </div>
              <h3 className="font-heading font-bold text-xl md:text-2xl text-white tracking-tight group-hover:text-empire-red transition-colors duration-200">{post.title}</h3>
